...
class NoisedDataGenerator(object):
    def __init__(self,
        ...
        self.kl_on_boundary_only = kl_on_boundary_only
    ...
    def convert_examples_to_dataset(self, examples, is_augmented=None, is_training=True):
        ...
        for (ex_index, example) in enumerate(examples):
                    ...
                    if not is_token_switched[cur_orig_index]:
                        ...
                        while noised_index < len(noised_tok_to_orig_index):
                            ...
                            if len(noised_tokens) + len(
                                ...
                                break
                            noised_tokens.append(noised_all_doc_tokens[noised_index])
                            noised_kl_mask.append(0)
                            noised_index += 1

                        if break_flag:
                            break

                    original_kl_mask.append(1 if not is_token_switched[cur_orig_index] else 0)

                assert len(noised_tokens) + len(
                    noised_truncated_query) + sequence_pair_added_tokens <= self.noised_max_seq_length

                if self.tokenizer.padding_side == "right":
                    noised_kl_mask = [0] * (len(noised_truncated_query) + 3) + noised_kl_mask + [0]
                    original_kl_mask = [0] * (len(original_truncated_query) + 3) + original_kl_mask + [0]
                else:
                    assert False

                noised_kl_mask += (self.noised_max_seq_length - len(noised_kl_mask)) * [0]
                original_kl_mask += (self.max_seq_length - len(original_kl_mask)) * [0]
            ...
            for (original_span, noised_span) in spans:
                ...
                original_span["kl_mask"][original_cls_index] = 1
                noised_span["kl_mask"][noised_cls_index] = 1
                ...
                all_original_kl_mask += [original_span["kl_mask"]]
                ...
                all_noised_kl_mask += [noised_span["kl_mask"]]
        ...
        all_noised_token_type_ids = torch.tensor([token_type_ids for token_type_ids in all_noised_token_type_ids],
                                                 dtype=torch.long)
        all_noised_kl_mask = torch.tensor([noised_kl_mask for noised_kl_mask in all_noised_kl_mask],
                                          dtype=torch.long)
        ...
        dataset = TensorDataset(all_original_input_ids, all_original_attention_mask, all_original_token_type_ids,
                                ...
                                all_noised_kl_mask, all_original_kl_mask, all_noised_start_positions,
...
def train(args, train_examples, train_dataset, model, stable_model, tokenizer, noised_data_generator=None):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if stable_model is not None:
                with torch.no_grad():
                    inputs["stable_model_start_logits"], inputs["stable_model_end_logits"] = stable_model(**inputs)[1:3]

            if noised_data_generator is not None:
                inputs.update({"noised_input_ids": batch[8], "noised_attention_mask": batch[9],
                               "noised_token_type_ids": batch[10], "noised_kl_mask": batch[11],
                               "original_kl_mask": batch[12], "noised_start_positions": batch[13],
...
def main():
    ...
    parser.add_argument("--kl_on_boundary_only", action="store_true",
                        help="Whether to enable kl loss on boundary only.")
    ...
    if args.enable_kl_loss or args.noised_loss or args.enable_translate_data or args.enable_data_augmentation:
        noised_data_generator = NoisedDataGenerator(
            ...
            kl_on_boundary_only=args.kl_on_boundary_only,
